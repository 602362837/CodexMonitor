...
export function GitDiffPanel({
  ...
  const pushErrorMessage = useMemo(() => {
    ...
    return `远端有新 commits。请先 Sync（先 pull 后 push）再重试。\n\n${pushError}`;
  ...
  const pushErrorAction = useMemo<SidebarErrorAction | null>(() => {
    ...
    return {
      label: _syncLoading ? "Sync 中..." : "Sync（先 pull 后 push）",
  ...
  const logCountLabel = logTotal
      ? `${logTotal} 个 commit`
    : logEntries.length
      ? `${logEntries.length} 个 commit`
      : "没有 commits";
  const logSyncLabel = logUpstream ? `↑${logAhead} ↓${logBehind}` : "未配置 upstream";
