...
describe("Home", () => {
  it("renders latest agent runs and lets you open a thread", () => {
    ...
    expect(screen.getByText("最近的 agents")).toBeTruthy();
  ...
  it("shows the empty state when there are no latest runs", () => {
    ...
    expect(screen.getByText("还没有 agent 活动")).toBeTruthy();
    expect(
      screen.getByText("开始一个线程后，最新回复会显示在这里。"),
    ).toBeTruthy();
  });

  it("opens the manual local path workspace prompt", () => {
    const onAddWorkspaceFromPath = vi.fn();
    render(<Home {...baseProps} onAddWorkspaceFromPath={onAddWorkspaceFromPath} />);

    fireEvent.click(screen.getByRole("button", { name: "从路径添加工作区" }));

    expect(onAddWorkspaceFromPath).toHaveBeenCalledTimes(1);
  });
  ...
  it("renders expanded token stats and account limits", () => {
    ...
    expect(screen.getByText("无限")).toBeTruthy();
    expect(screen.getByText("Pro")).toBeTruthy();
    expect(screen.getByText(/user@example\.com/)).toBeTruthy();
    expect(screen.queryByText("工作区 CodexMonitor")).toBeNull();
