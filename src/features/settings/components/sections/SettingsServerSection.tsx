...
export function SettingsServerSection({
  ...
  const tcpRunnerStatusText = (() => {
    ...
    if (tcpDaemonStatus.state === "error") {
      return tcpDaemonStatus.lastError ?? "移动端 daemon 处于错误状态。";
    }
  ...
  const handleAddRemoteConfirm = () => {
    void (async () => {
      ...
      try {
        ...
        setAddRemoteError(error instanceof Error ? error.message : "无法添加远端。");
      } finally {
  ...
  return (
    <SettingsSection
      ...
      subtitle={
        isMobileSimplified
          ? "填写桌面端设置里的 TCP host/token，然后运行连接测试。"
          : "配置 CodexMonitor 如何为移动端和远端客户端暴露 TCP 后端访问。桌面端默认仍保持本地使用，除非你明确切换到远端模式。"
      }
    >
      ...
      {!isMobileSimplified && (
        <div className="settings-field">
          ...
          >
            ...
            <option value="remote">远端（daemon）</option>
          </select>
          <div className="settings-help">
            本地模式会在桌面端进程内处理请求。远端模式会让桌面请求走移动客户端同一套 TCP 传输路径。
          </div>
        </div>
      )}
      ...
      <>
        {isMobileSimplified && (
          <>
            <div className="settings-field">
              <div className="settings-field-label">已保存远端</div>
              <div className="settings-mobile-remotes" role="list" aria-label="已保存远端">
        ...
        {!isMobileSimplified && (
          <SettingsToggleRow
            title="应用关闭后保持 daemon 运行"
            subtitle="关闭后，CodexMonitor 会在退出前停止托管的 TCP daemon 进程。"
          >
            <SettingsToggleSwitch
        ...
        <div className="settings-field">
          ...
          <div className="settings-help">
            {isMobileSimplified
              ? "使用桌面端 CodexMonitor 应用“服务器”设置中的 Tailscale 主机，例如 `macbook.your-tailnet.ts.net:4732`。"
              : "移动客户端和桌面端远程模式测试会使用这个 host/token。"}
          </div>
        </div>
        ...
        {isMobileSimplified && (
          <div className="settings-field">
            ...
            <div className="settings-help">
              请确认桌面端应用 daemon 正在运行，并且可通过 Tailscale 访问，然后重试此测试。
            </div>
          </div>
        )}
        ...
        {!isMobileSimplified && (
          <div className="settings-field">
            <div className="settings-field-label">移动访问 daemon</div>
            <div className="settings-field-row">
              <button
                type="button"
                className="button settings-button-compact"
              ...
              >
                {tcpDaemonBusyAction === "start" ? "正在启动..." : "启动 daemon"}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
                ...
                disabled={tcpDaemonBusyAction !== null}
              >
                {tcpDaemonBusyAction === "stop" ? "正在停止..." : "停止 daemon"}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
                ...
                disabled={tcpDaemonBusyAction !== null}
              >
                {tcpDaemonBusyAction === "status" ? "正在刷新..." : "刷新状态"}
              </button>
            </div>
            ...
            <div className="settings-help">
              从 iOS 连接前先启动此 daemon。它会使用当前 token，并监听与你配置主机端口一致的
        ...
        {!isMobileSimplified && (
          <div className="settings-field">
            ...
            <div className="settings-field-row">
              ...
              >
                {tailscaleStatusBusy ? "正在检查..." : "检测 Tailscale"}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
              ...
              >
                {tailscaleCommandBusy ? "正在刷新..." : "刷新 daemon 命令"}
              </button>
              <button
                type="button"
                className="button settings-button-compact"
              ...
              <>
                ...
                {!tailscaleCommandPreview.tokenConfigured && (
                  <div className="settings-help settings-help-error">
                    远程后端 token 为空。暴露 daemon 访问前请先设置 token。
                  </div>
                )}
              </>
            )}
      ...
      <div className="settings-help">
        {isMobileSimplified
          ? "仅使用你自己的基础设施。在 iOS 上，请从桌面端 CodexMonitor 设置中获取 Tailscale 主机名和 token。"
          : "移动访问应限定在你自己的基础设施（tailnet）内。CodexMonitor 不提供托管后端服务。"}
      </div>
      {addRemoteOpen && (
        <ModalShell
        ...
        >
          <div className="settings-add-remote-header">
            <div className="settings-add-remote-title">添加远端</div>
            <button
              type="button"
      ...
      {pendingDeleteRemote && (
        <ModalShell
          ...
          ariaLabel="删除远端确认"
        >
          <div className="settings-delete-remote-title">删除远端？</div>
          <div className="settings-delete-remote-message">
            要从已保存远端中移除 <strong>{pendingDeleteRemote.name}</strong> 吗？这只会从此设备移除该配置。
          </div>
