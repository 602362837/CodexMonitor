...
export function WorktreePrompt({
  ...
  return (
    ...
    >
      <div className="ds-modal-title worktree-modal-title">新建工作树 agent</div>
      ...
      >
        ...
        {branchMenuOpen && (
          <BranchList
            ...
            emptyText={
              branch.trim().length > 0 ? "没有匹配分支" : "没有找到分支"
            }
            onMouseEnter={(index) => {
              setDidNavigateBranches(true);
      ...
      <div className="worktree-modal-hint">
        保存在项目上（设置 → 环境），并在每个新工作树创建后于专用终端中运行一次。
      </div>
      <textarea
